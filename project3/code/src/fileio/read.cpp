...
processCamera( Obj *child, Scene *scene )
{
    ...
    {
        scene->getCamera()->setLook( tupleToVec( getField( child, "viewdir" )),
...
static void processObject( Obj *obj, Scene *scene, mmap& materials )
{
	...
	if( name == "directional_light" ) {
		...
		PointLight* pl = new PointLight(scene, tupleToVec(getField(child, "position")),
											   tupleToVec(getColorField(child)) );
		
		if(hasField(child, "constant_attenuation_coeff") ) {
			pl->setConstant(getField(child, "constant_attenuation_coeff")->getScalar());
		}
		
		if(hasField(child, "linear_attenuation_coeff") ) {
			pl->setLinear(getField(child, "linear_attenuation_coeff")->getScalar());
		}

		if(hasField(child, "quadratic_attenuation_coeff") ) {
			pl->setQuad(getField(child, "quadratic_attenuation_coeff")->getScalar());
		}
		scene->add( pl );
	
	} else if(name == "ambient_light") {
		scene->ambient_light = tupleToVec(getField(child, "color"));
	}
		...
		WarnLight* wl = new WarnLight(scene, tupleToVec(getField(child, "position")),
											 tupleToVec(getColorField(child)),
											 tupleToVec(getField(child, "direction")),
											 getField(child, "angle_size")->getScalar(),
											 getField(child, "parameter")->getScalar() );
