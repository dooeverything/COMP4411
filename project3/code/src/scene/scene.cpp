...
vec3f Scene::getMaxThreshold()
{
	double threshold = traceUI->getThreshold();

	return vec3f(threshold, threshold, threshold);
}

int Scene::getNumSampling()
{
	return traceUI->getNumSampling();
}

bool Scene::IsSampling()
{
	return traceUI->IsSampling();
}

bool Scene::IsAdapSampling()
{
	return traceUI->IsAdapSampling();
}
