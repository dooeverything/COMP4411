...
void OriginalView::draw() // Draw original paint on the original view
{
	if(!valid())
	{
	...
	if ( m_pDoc->m_ucBitmap ) 
	{
		...
		m_nWindowWidth=w(); // The width of the widget 
		m_nWindowHeight=h(); // The Height of the widget
		...
		// OpenGL-Create a red cursor on original view //
		glPointSize(4.0f);
		glBegin(GL_POINTS);
		glColor3f(1.0, 0, 0);
		glVertex2d(coord.x, (double)h()-coord.y);
		glEnd();
	}
...
void OriginalView::getCursor(int x, int y)
{
	coord.x = x;
	coord.y = y;
	redraw();
