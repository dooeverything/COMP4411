...
void Camera::applyViewingTransform() {
	...
	//cout << "Camera Position is " << mPosition << endl;
	MyLookAt(mPosition, mLookAt, mUpVector);

}

void Camera::MyLookAt(Vec3f eye, Vec3f at, Vec3f up)
{
	
	Vec3f old_up(up[0], up[1], up[2]);
	
	Vec3f forward(at[0]-eye[0], at[1]-eye[1], at[2]-eye[2]);
	forward.normalize();
	//printf("Forward Vector: %f, %f, %f \n ", forward[0], forward[1], forward[2]);

	Vec3f right = forward ^ old_up;
	right.normalize();
	//printf("Right Vector: %f, %f, %f \n ", right[0], right[1], right[2]);

	Vec3f new_up = right ^ forward;
	//new_up.normalize();
	//printf("Up Vector: %f, %f, %f \n ", new_up[0], new_up[1], new_up[2]);

	GLdouble view_matrix[16] = { right[0], new_up[0], -forward[0], 0,
