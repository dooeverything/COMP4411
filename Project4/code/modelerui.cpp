...
void ModelerUI::currTime(float fTime) 
{
	...
	if (simulate()) {
		...
		/*ParticleSystem *pb = ModelerApplication::Instance()->GetBreathParticleSystem();

		if (pb != NULL) {
			float bakeStartTime = ModelerApplication::Instance()->GetBreathParticleSystem()->getBakeStartTime();
			float bakeEndTime = ModelerApplication::Instance()->GetBreathParticleSystem()->getBakeEndTime();
			if (bakeEndTime < 0.0f)
				bakeEndTime = fTime;
			indicatorRangeMarkerRange(bakeStartTime, bakeEndTime);
