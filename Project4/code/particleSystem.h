...
public:
	...
	virtual void drawBreathParticles(float t);
	void createBreathParticles(Vec3d pos, int n_Breath);

	// This fxn should save the configuration of all particles
	// at current time t.
	virtual void bakeRainParticles(float t);
	virtual void bakeBreathParticles(float t);
	...
	virtual void addBreathForce(Force* force) {
		breathForces.push_back(force);
	}
...
protected:
	...
	vector<Breath> breathParticles;
	
	vector<Force*> forces;
	vector<Force*> breathForces;

	/** Some baking-related state **/
	vector<vector<Rain>> rainBakingList;
	vector<vector<Breath>> breathBakingList;
