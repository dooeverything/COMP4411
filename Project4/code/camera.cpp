...
const float kMouseRotationSensitivity		= 0.25f/90.0f;
...
void Camera::clickMouse( MouseAction_t action, int x, int y )
{
	...
	float clipX = (2 * (float)x / width) - 1;
	float clipY = 1 - (2 * (float)y / height);
	...
	GLdouble objPosY;
	GLdouble objPosZ;
	gluUnProject(x, height - y, z, modelView, projView, viewPort, &objPosX, &objPosY, &objPosZ);

	

	/*if(mCurrentMouseAction )
	for (int i = 0; i < mds->ctrlPoints.size(); ++i) {
		if (abs(mds->ctrlPoints[i][0] - clipX) < 0.1 && abs(mds->ctrlPoints[i][0] - clipY) < 0.1)
		{
			cout << "Selected points at " << x << " and " << y << endl;
			selectedPoint = i;
			movePoint = true;
		}
	}*/

	if( mCurrentMouseAction == kActionCreateDot && mds->ctrlPoints.size() < 16 && selectedPoint == -1)
	{
		cout << width << " and " << height << endl;
		cout << x << " and " << y << endl;
		//Vec3d p(objPosX, objPosY, objPosZ);
		Vec3d p(mPosition[0], mPosition[1], mPosition[2]);
		mds->ctrlPoints.push_back(p);
	}

	/*if (mCurrentMouseAction == kActionDeleteDot && selectedPoint != -1) {
		mds->ctrlPoints.erase(mds->ctrlPoints.begin() + selectedPoint);
		movePoint = false;
	}*/
}
...
void Camera::dragMouse( int x, int y )
{
	...
	case kActionTranslate:
		{
			// Move only Left, Right, Up and Down 
			/*if (movePoint) {
				mds->ctrlPoints[selectedPoint][0] += mouseDelta[0];
				mds->ctrlPoints[selectedPoint][1] += mouseDelta[1];
				break;
			}*/
...
void Camera::applyViewingTransform() {
	if( mDirtyTransform )
		calculateViewingTransformParameters();
	//cout << "Camera Look at: " << mLookAt << endl;
	cout << "Position at: " << mPosition<< endl;
	//cout << "Up Vector: " << mUpVector << endl;
